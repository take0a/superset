...
テンプレートを有効にするには、`superset_config.py`で`ENABLE_TEMPLATE_PROCESSING` [機能フラグ](/docs/configuration/configuring-superset#feature-flags)を有効にする必要があります。テンプレートを有効にすると、仮想データセットやExploreのフィルターおよびメトリックコントロールのカスタムSQLにPythonコードを埋め込むことができます。デフォルトでは、以下の変数がJinjaコンテキストで使用可能になります:
...
末尾の `1 = 1` は、時間フィルターが設定されていない場合でも、`WHERE` 句に値が存在することを保証します。多くのデータベースエンジンでは、これを `true` に置き換えることができます。
...
Jinja テンプレートのデフォルト値は、SQL Lab ユーザーインターフェースの「Parameters」メニューから指定できます。UIでは、パラメータセットをJSONとして割り当てることができます。
...
`|where_in` フィルターを使用すると、SQL 文でロールを使用できます。例えば、`current_user_roles()` が `['admin', 'viewer']` を返す場合、次のテンプレートを使用します:
...
`dataset` マクロを使用して、物理データセットと仮想データセットをクエリできます。これは、データセットに計算列と指標を定義し、その定義をアドホック SQL Lab クエリで再利用したい場合に便利です。

マクロを使用するには、まずデータセットの ID を見つける必要があります。これは、すべてのデータセットが表示されているビューに移動し、対象のデータセットにマウスオーバーして URL を確認することで確認できます。たとえば、データセットの URL が https://superset.example.org/explore/?dataset_type=table&dataset_id=42 の場合、その ID は 42 です。
...
指標は集計であるため、結果のSQL式は指標以外のすべての列でグループ化されます。代わりに、グループ化する列のサブセットを指定することもできます:
...
デフォルトでは、このフィルターは値がnullの場合に `()` を（文字列として）返します。この場合、`default_to_none`パラメータを `True` に設定するとnullが返されます:
...
文字列を `datetime` オブジェクトとして読み込みます。これは日付操作を行う際に便利です。例えば:
